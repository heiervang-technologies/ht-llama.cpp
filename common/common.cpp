...
bool parse_cpu_range(const std::string & range, bool (&boolmask)[GGML_MAX_N_THREADS]) {
    ...
    if (dash_loc == std::string::npos) {
        LOG_ERR("%s", "Format of CPU range is invalid! Expected [<start>]-[<end>].\n");
        return false;
    }
    ...
    if (dash_loc == 0) {
        ...
        if (start_i >= GGML_MAX_N_THREADS) {
            LOG_ERR("%s", "Start index out of bounds!\n");
            return false;
        }
    }
    ...
    if (dash_loc == range.length() - 1) {
        ...
        if (end_i >= GGML_MAX_N_THREADS) {
            LOG_ERR("%s", "End index out of bounds!\n");
            return false;
        }
    }
...
void common_params_print_info(const common_params & params, bool print_devices) {
    ...
    LOG_INF("%s: verbosity = %d (adjust with the `-lv N` CLI arg)\n", __func__, common_log_get_verbosity_thold());

    // device enumeration creates a primary context on CUDA backends, skip it when the caller does not own any device
    if (print_devices) {
        LOG_TRC("%s", "device_info:\n");
        for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
            auto * dev = ggml_backend_dev_get(i);
            size_t free, total;
            ggml_backend_dev_memory(dev, &free, &total);
            LOG_TRC("  - %-8s: %s (%zu MiB, %zu MiB free)\n", ggml_backend_dev_name(dev), ggml_backend_dev_description(dev), total / 1024 / 1024, free / 1024 / 1024);
        }
    }
...
common_init_result::common_init_result(common_params & params, bool model_only) :
    ...
    for (auto & la : params.lora_adapters) {
        ...
        if (lora == nullptr) {
            LOG_ERR("failed to load lora adapter '%s'\n", la.path.c_str());
            return;
        }
    ...
    if (params.sampling.ignore_eos && llama_vocab_eos(vocab) == LLAMA_TOKEN_NULL) {
        LOG_WRN("%s", "vocab does not have an EOS token, ignoring --ignore-eos\n");
    ...
    if (lctx == NULL) {
        LOG_ERR("failed to create context with model '%s'\n", params.model.path.c_str());
        return;
    }
...
common_init_result_ptr common_init_from_params(common_params & params, bool model_only) {
    ...
    if (llama_pooling_type(lctx) == LLAMA_POOLING_TYPE_RANK) {
        ...
        if (llama_vocab_bos(vocab) == LLAMA_TOKEN_NULL) {
            LOG_WRN("%s", "vocab does not have a  BOS token, reranking will not work\n");
            ok = false;
        }
        ...
        if (!has_eos && !has_sep && !has_rerank_prompt) {
            LOG_WRN("%s", "vocab does not have an EOS token, SEP token, or rerank prompt. Reranking will not work\n");
            ok = false;
        } else if (!has_eos) {
            LOG_WRN("%s", "vocab does not have an EOS token, using SEP token as fallback\n");
        }
    ...
    if (params.warmup) {
        LOG_TRC("%s", "warming up the model with an empty run - please wait ... (--no-warmup to disable)\n");
...
common_context_seq_rm_type common_context_can_seq_rm(llama_context * ctx) {
    ...
    if (ret != 0) {
        LOG_ERR("llama_decode() failed: %d\n", ret);
        res = COMMON_CONTEXT_SEQ_RM_TYPE_NO;
        goto done;
    }

    if (llama_n_rs_seq(ctx) > 0) {
        LOG_TRC("%s", "the context supports bounded partial sequence removal\n");
...
static common_control_vector_data common_control_vector_load_one(const common_control_vector_load_info & load_info) {
    ...
    if (!ctx_gguf) {
        LOG_ERR("failed to load control vector file from %s\n", load_info.fname.c_str());
        return result;
    }

    int32_t n_tensors = gguf_get_n_tensors(ctx_gguf);
    if (n_tensors == 0) {
        LOG_WRN("no direction tensors found in %s\n", load_info.fname.c_str());
    }
    ...
    for (int i = 0; i < n_tensors; i++) {
        ...
        if (layer_idx < 0) {
            LOG_ERR("invalid/unparsable direction tensor layer index in %s\n", load_info.fname.c_str());
            result.n_embd = -1;
            break;
        } else if (layer_idx == 0) {
            LOG_ERR("invalid (zero) direction tensor layer index in %s\n", load_info.fname.c_str());
            result.n_embd = -1;
            break;
        }
        ...
        if (tensor->type != GGML_TYPE_F32) {
            LOG_ERR("invalid (non-F32) direction tensor type in %s\n", load_info.fname.c_str());
            result.n_embd = -1;
            break;
        }
        if (ggml_n_dims(tensor) != 1) {
            LOG_ERR("invalid (non-1D) direction tensor shape in %s\n", load_info.fname.c_str());
            result.n_embd = -1;
            break;
        }
        ...
        if (result.n_embd == -1) {
            ...
            LOG_ERR("direction tensor in %s does not match previous dimensions\n", load_info.fname.c_str());
            result.n_embd = -1;
            break;
        }
    ...
    if (result.n_embd == -1) {
        LOG_WRN("skipping %s due to invalid direction tensors\n", load_info.fname.c_str());
        result.data.clear();
    }
...
common_control_vector_data common_control_vector_load(const std::vector<common_control_vector_load_info> & load_infos) {
    ...
    for (const auto & info : load_infos) {
        ...
        if (result.n_embd != -1 && result.n_embd != cur.n_embd) {
            LOG_ERR("control vectors in %s does not match previous dimensions\n", info.fname.c_str());
            result.n_embd = -1;
            break;
        }
