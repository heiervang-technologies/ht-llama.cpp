...
async fn input(
    ...
    if body.text.is_some() && body.base64.is_some() {
        return err_response(
            StatusCode::BAD_REQUEST,
            "provide only one of `text` or `base64`",
        );
    }
...
mod tests {
    ...
    fn extract_token_returns_plain_value() {
        ...
        assert_eq!(extract_token("other=1&token=xyz"), Some("xyz".to_string()));
        assert_eq!(extract_token("token=xyz&other=1"), Some("xyz".to_string()));
    }
    ...
    fn extract_token_percent_decodes_safe_range() {
        ...
        assert_eq!(extract_token("token=a%20b%3Dc"), Some("a b=c".to_string()));
    }
    ...
    fn extract_token_decodes_plus_as_space() {
        ...
        assert_eq!(
            extract_token("token=hello+world"),
