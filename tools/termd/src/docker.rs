...
async fn write_files(state: &AppState, container_id: &str, files: &[CreateFile]) -> Result<()> {
    ...
    for f in files {
        ...
        // Bash-heredoc is too fragile across quoting; use base64
        // decoding so content passes through untouched.
        let script = format!(
            "set -e; mkdir -p \"$(dirname '{path}')\"; printf %s '{b64}' | base64 -d > '{path}'; chmod {mode:o} '{path}'"
        );
        let exec = state
            .docker()
            .create_exec(
                ...
                CreateExecOptions {
                    attach_stdout: Some(false),
                    attach_stderr: Some(false),
                    cmd: Some(vec!["/bin/bash".to_string(), "-c".to_string(), script]),
                    ..Default::default()
                },
            )
            .await
