...
#[command(
    name = "ht-termd",
    version,
    about = "Terminal-sandbox sidecar for heierchat tool execution."
)]
struct Args {
    ...
    /// on the WS upgrade. Leave unset only for loopback deployments;
...
async fn main() -> Result<()> {
    ...
    // Any peer that can reach a non-loopback socket gets a root shell
    // in a sandbox. Fail closed rather than relying on a firewall that
    // can change independently of this service.
    if !args.bind.is_loopback() && args.token.is_none() {
        anyhow::bail!(
            "refusing non-loopback bind {} without --token (or HT_TERMD_TOKEN)",
            args.bind
        );
    }
