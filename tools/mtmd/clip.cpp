...
ggml_tensor * clip_graph::resize_position_embeddings(uint32_t interpolation_mode) {
    ...
    GGML_ASSERT(pos_embd->ne[0] == n_embd);
    ...
    // A flattened 2D table may contain prefix tokens (typically CLS) before
    // the square spatial grid. Slice those before either returning or resizing.
    orig_w = (int64_t) std::sqrt(pos_embd->ne[1]);
    orig_h = orig_w;

    const int64_t expected_elements = orig_w * orig_h;
    if (pos_embd->ne[1] > expected_elements) {
        const int64_t extra_tokens = pos_embd->ne[1] - expected_elements;
