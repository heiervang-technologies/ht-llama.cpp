...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    for (ggml_type type : turboq_types) {
        for (bool v : {false, true}) {
            test_cases.emplace_back(new test_set_rows(GGML_TYPE_F32, type, GGML_TYPE_I64, { 256, 5, 1, 3 }, { 1, 1 }, 1, v));
            test_cases.emplace_back(new test_set_rows(GGML_TYPE_F32, type, GGML_TYPE_I64, { 256, 11, 1, 1 }, { 2, 3 }, 7, v));
        }
    }
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // TurboQuant KV caches use the shared attention-rotation graph. These cases
    // validate the fused codebook-domain path without applying a second transform.
    for (ggml_type type_KV : turboq_types) {
        for (int head_size : {64, 128, 256}) {
            test_cases.emplace_back(new test_flash_attn_ext(
                head_size, head_size, 4, {1, 1}, 512, 2, true, false,
                0, 0, GGML_PREC_F32, type_KV, type_KV));
        }
        test_cases.emplace_back(new test_flash_attn_ext(
            128, 128, 4, {1, 1}, 512, 2, true, false, 0, 0, GGML_PREC_F32, type_KV, GGML_TYPE_F16));
        test_cases.emplace_back(new test_flash_attn_ext(
            128, 128, 4, {1, 1}, 512, 2, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, type_KV));
    }
