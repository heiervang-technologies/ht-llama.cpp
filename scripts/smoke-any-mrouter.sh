...
#
# Usage:
#   scripts/smoke-any-mrouter.sh URL [URL ...]
#   MROUTER_PEERS="URL URL" scripts/smoke-any-mrouter.sh
#
...
if (( ${#PEERS[@]} == 0 )); then
    read -r -a PEERS <<< "${MROUTER_PEERS:-}"
fi
(( ${#PEERS[@]} > 0 )) || { echo "pass at least one URL or set MROUTER_PEERS" >&2; exit 2; }
...
check_models_endpoint() {
    ...
    echo "$body" | jq -e '.data | type == "array"' >/dev/null 2>&1 || return 3
...
for peer in "${PEERS[@]}"; do
    ...
    case $rc in
        ...
        10) printf "→ %sany route FAIL (4xx: %s — prime a model first)%s\n" "$RED" "$resolved" "$RESET"; ((fail_count+=1)) ;;
    esac
done
