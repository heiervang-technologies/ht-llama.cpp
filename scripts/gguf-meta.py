...
#   gguf-meta.py --check-instruct FILE      # reject non-GGUF, structurally
#                                           #   truncated, non-gemma4, or base targets
#
...
def _rd(f, fmt, n):
    data = f.read(n)
    if len(data) != n:
        raise EOFError(f"truncated GGUF metadata: needed {n} bytes, found {len(data)}")
...
def _rstr(f):
    ln = _rd(f, '<Q', 8)
    if ln > 16 * 1024 * 1024:
        raise ValueError(f"unreasonable GGUF string length: {ln}")
    data = f.read(ln)
    if len(data) != ln:
        raise EOFError(f"truncated GGUF string: needed {ln} bytes, found {len(data)}")
...
def _rval(f, t):
    ...
    if t == T_ARRAY:
        ...
        ln = _rd(f, '<Q', 8)
        if ln > 100_000_000:
            raise ValueError(f"unreasonable GGUF array length: {ln}")
        out = []
        for i in range(ln):
...
def read_meta(path):
    ...
    Raises on a non-GGUF or structurally truncated file. Also walks tensor-info
    offsets and verifies the file reaches the start of its final tensor. This
    catches stubs and gross truncation, but cannot prove the final tensor payload
    is complete without a full GGML-type size table. Sets _data_complete as this
    conservative structural check and records _min_size_bytes."""
    import os
    meta = {}
    ...
    with open(path, 'rb') as f:
        ...
        if n_tensors > 10_000_000 or n_kv > 1_000_000:
            raise ValueError(f"unreasonable GGUF counts: n_tensors={n_tensors}, n_kv={n_kv}")
        meta["_version"] = ver
...
def is_instruct(meta):
    ...
    return (ft == "it") or ("instruct" in ft) or ("-it" in name) or name.endswith(" it") or \
...
def main():
    ...
    if args[0] == "--check-instruct":
        ...
        if meta.get("_data_complete") is False:
            ...
            print(f"REJECT {path}: TRUNCATED — header valid but file does not reach its final tensor "
