...
#
# Env:
#   DFLASH_PARITY_TARGET     required target GGUF path
#   DFLASH_PARITY_DRAFTER    required drafter GGUF path
...
TARGET="${DFLASH_PARITY_TARGET:-}"
DRAFTER="${DFLASH_PARITY_DRAFTER:-}"
...
[[ -n "$TARGET" ]] || { echo "DFLASH_PARITY_TARGET is required" >&2; exit 1; }
[[ -n "$DRAFTER" ]] || { echo "DFLASH_PARITY_DRAFTER is required" >&2; exit 1; }
...
for i in $(seq 0 $((N_PROMPTS - 1))); do
    ...
    [[ "$id" =~ ^[A-Za-z0-9._-]+$ ]] || { echo "unsafe prompt id: $id" >&2; exit 1; }
    err="$ERRDIR/$id.err"
    ...
    timeout "$PROMPT_TIMEOUT" "${NICE_PREFIX[@]}" "$BIN" \
        -m "$TARGET" -md "$DRAFTER" \
        --spec-type draft-dflash --spec-draft-n-max "$N_MAX" \
        -p "$text" -n "$n_predict" \
