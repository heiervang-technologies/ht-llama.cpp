...
**3 · Build flags / env:**
```
-DGGML_CUDA=ON  -DCMAKE_CUDA_ARCHITECTURES=61  -DGGML_CUDA_FORCE_MMQ=ON
-DCMAKE_CUDA_COMPILER=/opt/cuda-pascal-runfile/bin/nvcc
-DCMAKE_CUDA_HOST_COMPILER=/opt/gcc-14/usr/bin/g++
```
Current llama.cpp has no `GGML_CUDA_F16` build switch. `FORCE_MMQ=ON` selects the int8 `__dp4a` path; audit new kernels to keep FP32 accumulation rather than enabling raw FP16 arithmetic on Pascal. Runtime env (drop in `/etc/profile.d/cuda-pascal.sh`):
```
...
> **Verified reference tarballs:** members rooted at `/` (`opt/ht-llama-cuda/` + `opt/cuda-pascal-runfile/`) → `tar -C / -x`. Host-provided (correctly absent): `libstdc++.so.6`, `libgomp.so.1`, `libcuda.so.1` (from `gcc-libs` + `nvidia-580xx`).
>
...
cd "${HT_LLAMA_SRC:?set HT_LLAMA_SRC to the ht-llama.cpp checkout}"
...
LD_LIBRARY_PATH=/opt/cuda-pascal-runfile/lib64:/opt/cuda-pascal-runfile/targets/x86_64-linux/lib \
  ./build-cuda/bin/llama-bench --list-devices 2>&1 | grep ggml_cuda_init
