...
PASCAL_TMP="${PASCAL_TMP:-$HOME/big-tmp}"
sh ~/.cache/yay/cuda-pascal/cuda_12.9.1_575.57.08_linux.run \
    --extract="$PASCAL_TMP/cuda-x" \
    --tmpdir="$PASCAL_TMP"
sudo mkdir -p /opt/cuda-pascal-runfile
for d in "$PASCAL_TMP"/cuda-x/cuda_* \
         "$PASCAL_TMP"/cuda-x/libcublas \
         "$PASCAL_TMP"/cuda-x/libcusparse \
         "$PASCAL_TMP"/cuda-x/libcurand \
         "$PASCAL_TMP"/cuda-x/libcusolver \
         "$PASCAL_TMP"/cuda-x/libcufft \
         "$PASCAL_TMP"/cuda-x/libnvfatbin \
         "$PASCAL_TMP"/cuda-x/libnvjitlink; do
...
- Current llama.cpp has no `GGML_CUDA_F16` build switch. Do not invent one: force MMQ for quantized matmul and audit new kernels so Pascal keeps FP32 accumulation instead of raw FP16 arithmetic.
...
## Sources
- Primer: `scripts/quadro-p5200-llamacpp-primer.md`
