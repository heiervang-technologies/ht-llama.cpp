...
GGUFS_DIR="${GGUFS:-${MODELS:-}}"
DRAFTER_DIR="${DFLASH_DRAFTER_DIR:-${GGUFS_DIR:+$GGUFS_DIR/dflash-gemma4-31b-gguf}}"
TS="$(date +%Y%m%d-%H%M%S)"
...
[[ -n "$DRAFTER_DIR" ]] || { echo "set DFLASH_DRAFTER_DIR, GGUFS, or MODELS" >&2; exit 1; }
...
resolve_target() {
    ...
    if [[ -n "$GGUFS_DIR" && -f "$GGUFS_DIR/$t" ]]; then echo "$GGUFS_DIR/$t"; return 0; fi
    return 1
}
...
run_one() {  # target_path drafter_path prompt seed out_dir label
    ...
    timeout 360 "$BIN" \
        -m "$tgt" -md "$dft" --spec-type draft-dflash \
        -p "$prompt" \
...
REJECT_LOG="$(mktemp /tmp/dflash-target-sweep-reject.XXXXXX)"
...
for tname in "${TGT_ARR[@]}"; do
    ...
    tlabel="$(basename "$tname" .gguf)"
    if ! python3 "$META" --check-instruct "$tpath" >/dev/null 2>"$REJECT_LOG"; then
        reason="$(tr -s ' ' < "$REJECT_LOG")"
    ...
    for plabel in "MT-Bench" "HumanEval"; do
        ...
        accs=""; last_tps="NA"
        for ((r=0; r<RUNS; r++)); do
            if awk -v t="$TEMP" 'BEGIN { exit !(t == 0) }'; then
                seed="$SEED_BASE"
            else
                seed=$((SEED_BASE + r))
            fi
            label="${tlabel}_${plabel}_${r}"
